...
const joinedSequence = (parsers: Parser<string, string, unknown>[]) =>
  sequenceOf(parsers).map((s) => s.join(""));
// Match many of parsers, then join results
const joinedMany = (parser: Parser<unknown, string, unknown>) =>
  many(parser).map((s) => s.join(""));
// Between helpers
...
const bindingSymbol = coroutine((run) => {
  ...
  const bindingMap = run(getData as Parser<unknown, string, unknown>) as Record<
    string,
    symbol
  >;
...
const returnBinding = pipeParsers([
  optionalWhitespace,
  str("->"),
  optionalWhitespace,
...
const paddedSyncLine = pipeParsers([
  between(optionalWhitespace)(toEndOfLine)(syncLine),
]);
// Each block starts with a when clause
const whenBlock = pipeParsers([
  optionalWhitespace,
  str("when"),
  whitespace,
  many(paddedSyncLine),
